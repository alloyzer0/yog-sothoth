...
_Static_assert(offsetof(ys_runtime_desc, device_id) == 56, "ys_runtime_desc.device_id offset changed");
...
_Static_assert(offsetof(ys_frame_request, feature_profile) == 48,
...
_Static_assert(offsetof(ys_frame_status, submit_sequence) == 48,
               "ys_frame_status.submit_sequence offset changed");

int main(void) {
    printf("{\n");
    printf("  \"pointer_size\": %zu,\n", sizeof(void*));
    printf("  \"ys_struct_header\": {\"size\": %zu, \"align\": %zu, \"next\": %zu},\n",
           sizeof(ys_struct_header), _Alignof(ys_struct_header), offsetof(ys_struct_header, next));
    printf("  \"ys_runtime_desc\": {\"size\": %zu, \"align\": %zu, \"device_id\": %zu},\n",
           sizeof(ys_runtime_desc), _Alignof(ys_runtime_desc), offsetof(ys_runtime_desc, device_id));
    printf("  \"ys_error_info\": {\"size\": %zu, \"align\": %zu, \"message\": %zu},\n",
           sizeof(ys_error_info), _Alignof(ys_error_info), offsetof(ys_error_info, message));
    printf("  \"ys_frame_request\": {\"size\": %zu, \"align\": %zu, \"feature_profile\": %zu},\n",
           sizeof(ys_frame_request), _Alignof(ys_frame_request), offsetof(ys_frame_request, feature_profile));
    printf("  \"ys_frame_status\": {\"size\": %zu, \"align\": %zu, \"submit_sequence\": %zu}\n",
           sizeof(ys_frame_status), _Alignof(ys_frame_status), offsetof(ys_frame_status, submit_sequence));
    printf("}\n");
    return 0;
}
